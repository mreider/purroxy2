...
function ErrorState({ path, error, onOpenSettings, onRetry }) {
  const errMsg = error || "OS error 13: Permission denied (os error 13)";
  return (
    <div className="content">
      <div className="content-inner">
        <div className="section-bar">
        ...
        <div className="error-card">
          ...
          <div className="path-block" style={{ color: "var(--status-fail)" }}>{errMsg}</div>
          <div className="error-card-actions">
            <button className="btn btn-primary" onClick={onOpenSettings}>
              <Icon name="settings" /> Open Settings
            </button>
            <button className="btn" onClick={onRetry}>Retry</button>
            <button className="btn btn-ghost" onClick={() => navigator.clipboard?.writeText(errMsg)}><Icon name="copy" /> Copy error</button>
          </div>
        </div>
      </div>
    </div>
